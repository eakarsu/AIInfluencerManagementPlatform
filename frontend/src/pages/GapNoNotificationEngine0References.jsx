...
// Auto-generated page for: No notification engine (0 references)

export default function GapNoNotificationEngine0References() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-notification-engine-0-references', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>No notification engine (0 references)</h1>
      <p style={{ color: '#666', marginBottom: '1.5rem' }}>No notification engine (0 references)</p>
      <form onSubmit={handleSubmit} style={{ marginBottom: '1.5rem' }}>
        ...
        >
          {loading ? 'Processing...' : 'Run No notification engine (0 references)'}
        </button>
      </form>
